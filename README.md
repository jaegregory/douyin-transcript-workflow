...
如果把仓库 clone 到别的位置，需要同步修改：

- `douyin_launcher.py` 里的 `pipelinePath`
- `douyin_pipeline.py` 顶部的 `COOKIE_PATH`、`OUTPUT_DIR`、`TRANSCRIPT_DIR`、`MEDIA_DIR`
...
如果使用 CUDA 转写，需要本机 CUDA / GPU 环境能被 `ctranslate2` 和 `faster-whisper` 正常识别。脚本默认优先 CUDA，失败后回退 CPU。
